...
const DatabaseTabs = ({
  ...
  useEffect(() => {
    if (selectedPath?.table && selectedPath?.database && activeTab === 'profiling') {
      fetchProfilingData();
    }
  ...
  const fetchProfilingData = async () => {
    ...
    try {
      ...
      setProfilingError(error.response?.data?.error || 'Failed to generate profile');
    } finally {
  ...
  const tabs = selectedPath?.table
    ? [...baseTabs, { id: 'profiling', label: 'Data Profiling', icon: 'analytics' }]
    : baseTabs;
